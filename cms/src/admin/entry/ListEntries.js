...
class ListEntries extends Component {
  ...
  async _getEntryList() {
    let response = await axiosInstance.get(
      `/admin/${this.state.structureSlug}/listentries`
    );
    this.setState({
  ...
  async _deleteEntry(entry) {
    ...
    let response = await axiosInstance.delete("/admin/deleteentry", {
      data: payload
    });
    this._getEntryList();
  }
  ...
  render() {
    ...
    if (this.state.entryList.length > 0) {
                ...
                <tr key={index}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => this._deleteEntry(entry)}
                    >
                      Delete
                    </button>
                  </td>
      ...
      return <div className="error">No Entries yet !!</div>;
    }
  }
}
